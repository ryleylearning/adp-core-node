...
function validate(data, payload, metaPaths, dataPath) {
	...
	function executeValidation() {
		...
		payloadDataPaths.forEach(function payloadDataPathsForEachCb(payloadDataPath) {
			...
			var metaObj = _.where(metaPaths, {path: metaDataPath})[0];
			if(metaObj && metaObj.meta) {
				...
				if(metaObj.meta.optional === false) {
					if(!optionalValidate(metaObj.meta.optional, validateObj)) {
						failures.push(new EventException({description: 'Meta validation failure', path: payloadDataPath, message: 'Value is not optional and must be set.', value: validateObj}));
					}
				}
				if(metaObj.meta.minLength && metaObj.meta.maxLength) {
					if(!minMaxLengthValidate(metaObj.meta.minLength, metaObj.meta.maxLength, validateObj)) {
						failures.push(new EventException({description: 'Meta validation failure', path: payloadDataPath, message: 'Value is not within acceptable range between ' + metaObj.meta.minLength + ' and ' + metaObj.meta.maxLength, value: validateObj}));
					}
				}
				if(_.isNumber(metaObj.meta.minItems) && _.isNumber(metaObj.meta.maxItems)) {
					if(!minMaxItemsValidate(metaObj.meta.minItems, metaObj.meta.maxItems, validateObj)) {
						failures.push(new EventException({description: 'Meta validation failure', path: payloadDataPath, message: 'Collection size is not within acceptable range between ' + metaObj.meta.minItems + ' and ' + metaObj.meta.maxItems, value: validateObj}));
					}
				}
			}
		});
