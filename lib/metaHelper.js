...
function get(conn, apicall, payload, cb) {
	...
	function extractRefs(conn, meta, hrefs) {
		if(!meta) return
		...
		Object.keys(meta).forEach(function keysForEachCb(key) {
			if(~lookFor.indexOf(key)) {
				...
				if(meta[key] && typeof meta[key] === 'object') extractRefs(conn, meta[key], outputHrefs);
			}
		});
		return outputHrefs;
		// saveLocalMeta(meta);
	...
	function appendSubsets(err, results) {
	...
	if(!metaObject) {
		getHttpMeta(conn, apicall.metaPath, apicall.methodName, function getHttpMetaCb(err, meta) {
			rawMeta = meta;
			if(rawMeta) {
				var hrefs = extractRefs(conn, rawMeta);
...
function validate(conn, data, payload, metaObject, dataPath, cb) {
		...
		getHttpMeta(conn, href, 'AsyncValidation.' + href, function getHttpMetaCb(err, asyncCodeList) {
			if(asyncCodeList) {
				if(!codeListsValidate(asyncCodeList, listInfo.data)) {
					var validationResult = new EventException({description: 'Async List of values must match', path: listInfo.path, message: 'Collection values did not match list of acceptable values.', value: listInfo.data});
	...
	*/
	function parseForValidation(validationData, validationMeta, validationDataPath) {
		var allfailures = [];
		var payloadDataPaths = [validationDataPath];
		if(typeof validationData !== 'string') {
			payloadDataPaths = getPayloadDataPaths(validationData, validationDataPath || '');
			payloadDataPaths.push(validationDataPath);
		}
	...
	results = results.concat(parseForValidation(data, metaObject, dataPath));
