	...
	function executeValidation() {
		...
		payloadDataPaths.forEach(function payloadDataPathsForEachCb(payloadDataPath) {
			...
			payloadProps.forEach(function payloadPropsForEachCb(payloadProp) {
				if(_.isNumber(payloadProp * 1) && !_.isNaN(payloadProp * 1)) payloadProp = '__arrayIndex__';
				metaDataPath.push(payloadProp);
			});
			metaDataPath = metaDataPath.join('/');
			// console.log('tmpPayloadDataPath', tmpPayloadDataPath);
...
function get(opts, cb) {
	getMeta(opts.conn, opts.apiInfo, function getMetaCb(err, newMeta) {
		var metaObject = getMetaObject(newMeta, opts.payload);
