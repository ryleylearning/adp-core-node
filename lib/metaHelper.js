...
function get(conn, apicall, payload, cb) {
	var rawMeta;
	var metaFileName = conn.accessToken + apicall.metaPath.replace(/\//g, '.') + '.json';

	function getLocalMeta(getCb) {
		function parse(str) {
			return JSON.parse(str);
		}
		function catchParseError(e) {
			debug('Unable to parse JSON found in `' + metaFileName + '`. Exception: ' + e);
			getCb();
		}
		var file = metacache.file(metaFileName);
		file && file.async('string')
								.then(parse)
								.then(getCb)
								.catch(catchParseError);
		!file && getCb();
	}

	function saveLocalMeta(meta) {
		metacache.file(metaFileName, JSON.stringify(meta));
		debug('Caching meta rules with file name `' + metaFileName + '`');
	...
	function getMetaSubset(ref, asyncCb) {
		getHttpMeta(conn, ref.href, 'Embedded.CodeList' + ref.href, function getHttpMetaCb(err, subsetmeta) {
			...
			var newListItems;
			if(subsetmeta && subsetmeta.codeLists) {
				var concatItems = [];
				subsetmeta.codeLists.forEach(function codelistsForEachCb(list) {
					if(list.listItems && list.listItems.length > 0) concatItems = concatItems.concat(list.listItems);
				});
				if(concatItems.length > 0) newListItems = concatItems;
			}
	...
	function appendSubsets(err) {
		var metaObject = getMetaObject(rawMeta, payload);
		saveLocalMeta(metaObject);
		cb(err, metaObject);
	}
	getLocalMeta(function getLocalMetaCb(localMeta) {
		if(!localMeta) {
			getHttpMeta(conn, apicall.metaPath, apicall.methodName, function getHttpMetaCb(err, meta) {
				rawMeta = meta;
				if(rawMeta) {
					var hrefs = extractRefs(rawMeta);
					return async.each(hrefs, getMetaSubset, appendSubsets);
				}
				cb(err);
			});
		} else {
...
function getUnvalidatedPaths(payloadDataPaths, validationMeta) {
	validationMeta.forEach(function validationMetaForEachCb(meta) {
		// if(meta.rulesExecuted) return;
		var modifiedMetaPath = meta.path.replace(/__arrayIndex__/g, '0');
		if(!~payloadDataPaths.indexOf(modifiedMetaPath)) payloadDataPaths.push(modifiedMetaPath);
	});
	return payloadDataPaths;
}

function validate(conn, payload, metaObject, cb) {
	var metaObjectCopy = JSON.parse(JSON.stringify(metaObject));
	var results = [];
	...
	*/
	...
	/**
		...
		- Different from other "Validate" functions (above):
			...
			** finds all validiations and passes the "metaObj" to `parsePayloadForValidation` which runs the actual validations.
			** always passes the full payload for validation to `parsePayloadForValidation` because the dependencies
			need to evaluate across the whole payload.
	*/
	function dependenciesValidate(dep, val) {
		var depfailures = [];
		if(objectPath(dep, 'pattern/oneOf')) {
			var pats = dep.pattern.oneOf;
			pats.forEach(function patsForEachCb(pat) {
				if(patternValidate(pat.value, val)) {
					depfailures = depfailures.concat(parsePayloadForValidation(payload, pat.attributes.metaObj));
				}
			});
		}
		return depfailures;
	...
	*/
	function asyncCodeListValidation(listInfo, asyncCb) {
		...
		getHttpMeta(conn, href, 'AsyncValidation.' + href, function getHttpMetaCb(err, asyncCodeList) {
			if(asyncCodeList) {
				var combined = {listItems: []};
				if(asyncCodeList.codeLists) {
					asyncCodeList.codeLists.forEach(function combine(list) {
						if(list.listItems) combined.listItems = combined.listItems.concat(list.listItems);
					});
				} else {
					combined = asyncCodeList;
				}
				if(!codeListsValidate(combined, listInfo.data)) {
	...
	*/
	...
	/**
		...
		- Called from a loop in `parsePayloadForValidation`
	...
	*/
	...
	function shouldValidate(rules, validateObj) {
		var tmpObj = validateObj;
		if(rules.optional) {
	...
	function runValidations(metaObj, validateObj, payloadDataPath) {
		...
		if(!shouldValidate(metaObj.meta, validateObj)) return failures;
	...
	*/
	function parsePayloadForValidation(validationData, validationMeta) {
		var allfailures = [];
		var payloadDataPaths = getPayloadDataPaths(validationData);
		payloadDataPaths = getUnvalidatedPaths(payloadDataPaths, validationMeta);
	...
	results = results.concat(parsePayloadForValidation(payload, metaObjectCopy));
...
module.exports = {
	...
	*/
	...
	getPayloadDataPaths: getPayloadDataPaths
};
