...
function APIProductHelper(schemaLocation) {
	...
	this.sync = function sync() {
		async.each(events, this.executeEvent.bind(this), this.eventResponse.bind(this));
	};

	this.executeEvent = function executeEvent(eventInfo, cb) {
		var apiProductInstance = eventInfo.apiProductInstance;
		apiProductInstance.call(eventInfo.methodName, {payload: eventInfo.eventInstance.getPayload()}, cb);
	};

	this.eventResponse = function eventResponse(err) {
