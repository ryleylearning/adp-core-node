...
function APIRequest(opts, cb) {
	...
	if(opts.httpMethod === 'GET') {
		request.get({uri: opts.url, qs: opts.qs, headers: opts.headers, agentOptions: opts.agentOptions, strictSSL: false}, response);
	} else if(opts.httpMethod === 'POST') {
		request({url: opts.url, headers: opts.headers, agentOptions: opts.agentOptions, method: 'POST', json: true, body: opts.payload, strictSSL: false}, response);
	} else if(opts.httpMethod === 'DELETE') {
		request.del({url: opts.url, headers: opts.headers, agentOptions: opts.agentOptions, strictSSL: false}, response);
	}
}
