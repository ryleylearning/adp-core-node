...
/**
@class ConsumerApplicationInstance
*/
function ConsumerApplicationInstance(conn, app) {
	...
	function saveEvent(payload, cb) {
		...
		var eventInstance = _.where(events, {eventId: payload.eventId})[0];
		if(!eventInstance || !'validate' in (eventInstance || {})) return cb(new ReferenceError('EventId not found or invalid eventId. EventId must not be altered or deleted from payload.'));
		...
		eventInstance.validate('', function validateCb(valerr, validationResults) {
			...
			delete eventInstance.getPayload().eventId;
