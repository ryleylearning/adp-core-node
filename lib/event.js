...
function Event(opts) {
	...
	var serviceCategoryCode = canonicalUri.substring(1, canonicalUri.indexOf('/') - 1);
	...
	function setServiceCategoryCode(code) {
		serviceCategoryCode = code;
		var pathProps = serviceCategoryCodePath.split('/');
		var pathProp = pathProps.pop();
		var serviceCategoryCodeObj = objectPath(payload, pathProps.join('/'));
		serviceCategoryCodeObj[pathProp] = serviceCategoryCode;
	}

	function setEventNameCode(code) {
		eventNameCode = code;
		var pathProps = eventNameCodePath.split('/');
		var pathProp = pathProps.pop();
		var eventNameCodeObj = objectPath(payload, pathProps.join('/'));
		eventNameCodeObj[pathProp] = eventNameCode;
	}
	...
	setEventNameCode(eventNameCode);
