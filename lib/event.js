...
function Event(opts) {
	...
	this.appInstance = opts.appInstance;
	...
	this.initFinish = function initFinish(err) {
		if(typeof this.dataPath !== 'undefined' && this.dataPath.length > 0) this.appendData(this.initObj);
	...
	this.setPayload = function setPayload(p) {
		payload = p;
	};
	...
	this.validate = function validate(partialPath, cb) {
		...
		var conn = this.appInstance.getConnection();
