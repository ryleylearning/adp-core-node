...
function validateAgainstMeta(meta, path, data) {
	...
	var dataObj = objectPath(data, metaRoot);
	function searchMeta(obj) {
		Object.keys(obj).forEach(function keysForEach(key) {
			console.log(key, key.indexOf('/'), _.isNumber(key));
			if(key.indexOf('/') > -1) {
				var tmpkey;
				if(key[0] === '/') {
					tmpkey = key.substring(1, key.length);
				}
				var tmpObject = objectPath(dataObj, tmpkey);
				console.log('dataObj', tmpObject);
				searchMeta(obj[key]);
			} else if(_.isNumber(parseInt(key))) {
				var tmpObject = objectPath(dataObj, key);
				console.log('dataObj', tmpObject);
				searchMeta(obj[key]);
			} else {
