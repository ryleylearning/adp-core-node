	...
	if (_.isUndefined(output)) {
		if (!_.isUndefined(schema.default)) {
			...
			if (_.isArray(schema.items)) {
				...
				log.info('Array type\'s \'items\' property is not an array or an object, cannot process');
			}
		}
	}
...
*/
function defaultPayload(apiInfo, schemaLocation) {
	var schemaName = apiInfo.schemaName;

	try{
		fs.statSync(schemaLocation + schemaName + '.json');
	} catch(e) {
		log.error('Invalid schema location `' + schemaLocation + schemaName + '.json`');
		return {};
	}

	var schema = require(schemaLocation + schemaName + '.json');
